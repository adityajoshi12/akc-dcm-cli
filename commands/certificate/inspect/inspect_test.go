package inspect_test

import (
	"bytes"
	"os"
	"testing"

	"github.com/adityajoshi12/akc-dcm-cli/commands/certificate/inspect"
)

func createTempCertFile(t *testing.T, content string) string {
	t.Helper()
	tempFile, err := os.CreateTemp("", "cert-*.crt")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}

	if _, err := tempFile.WriteString(content); err != nil {
		t.Fatalf("failed to write to temp file: %v", err)
	}

	tempFile.Close()
	return tempFile.Name()
}

func TestInspectCommand_Validate(t *testing.T) {
	cmd := inspect.InspectCommand{}

	// Test with no CertPath
	if err := cmd.Validate(); err == nil {
		t.Fatalf("expected error due to missing certificate path, got nil")
	}

	// Test with CertPath
	cmd.CertPath = "dummy-cert-path"
	if err := cmd.Validate(); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
}

func TestInspectCommand_Run(t *testing.T) {
	// Create a temporary certificate file
	validCert := createTempCertFile(t, `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`)
	defer os.Remove(validCert)

	cmd := inspect.InspectCommand{
		CertPath: validCert,
	}

	if err := cmd.Run(); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
}

func TestNewInspectCommand(t *testing.T) {
	// Create a temporary certificate file with valid content
	validCert := createTempCertFile(t, `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`)
	defer os.Remove(validCert)

	cmd := inspect.NewInspectCommand()

	if cmd.Use != "inspect" {
		t.Errorf("expected command use to be 'inspect', got %s", cmd.Use)
	}

	if cmd.Short != "Inspect certificate" {
		t.Errorf("expected command short description to be 'Inspect certificate', got %s", cmd.Short)
	}

	if cmd.Long != "Parse detail of certificate" {
		t.Errorf("expected command long description to be 'Parse detail of certificate', got %s", cmd.Long)
	}

	output := &bytes.Buffer{}
	cmd.SetOut(output)
	cmd.SetErr(output)
	cmd.SetArgs([]string{"--cert=" + validCert})

	err := cmd.Execute()
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
}
